    ...
    +, Tổng quan:
        +, Các thiết bị phần cứng được truy cập bởi user thông qua các file đặc biệt nhờ kernel, gọi là Device file. Các file này được tập hợp trong thư mục /dev
            --> Các System Calls open/read/write/close/lseek tác động vào Device file sẽ được chuyển hướng bởi hệ điều hành tới các Device Driver tương ứng của các Hardware.
            --> Khi ta đọc ghi vào 1 Device file, kernel sẽ biết chính xác ta muốn đọc ghi vào Device nào.
        +, Có 2 kiểu Device file: 
            +, Character Device
            +, Block Device
    ...
    --> Khi ta muốn đọc ghi vào thanh ghi để điều khiển dữ liệu, ta có thể đọc ghi vào device file
        +, Driver đại diện bởi struct cdev
        +, Device(thiết bị của mình) được đại diện bởi Device file
            --> Driver giúp cho thiết bị chạy được.

        
        +, Bước 3: Register FIle Operation
            +, Vậy để có thể đọc ghi vào device file đó, ta cần một bước là bước Register FIle Operation
